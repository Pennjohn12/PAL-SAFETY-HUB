...
- Package 6 trusted-rescan scanner digest `sha256:ab00939cb01de07150d20ad643dab338feb19fb31f0033049ad4a6f18e457da5` passed Artifact Analysis and is active only in private, min-0/max-1/concurrency-1 Staging. Three false-positive actions, private callback, and narrow identities/IAM are ACTIVE; anonymous boundaries fail closed. The corrected metadata handoff passed the complete authenticated two-person synthetic regression: clean exact-object rescan, same-person denial, different-Admin approval, generation-bound exact download, four-event audit validation, and complete cleanup. A strict stored-event audit recomputation helper correction passes 91/91 locally and does not change deployed event creation. Production and real data remain prohibited; existing-file planning, billing reconciliation, the verifier deployment decision, and an exact Production proposal remain open.
...
- Package 6 of 15, sensitive vault and safe release: **In progress / scanner, signed-download, two-person approval, live audit-chain, dedicated identity, retention, legal-hold, deletion, internal-notification, and false-positive release gates passed in Staging** on 2026-08-29. The accepted no-finding scanner runs privately against four isolated buckets and passed clean/quarantine, size, duplicate-generation, definition schedule, memory, timeout recovery, and malformed-event tests. Three vault callables and the hourly retention worker use a dedicated zero-key least-privilege identity. Synthetic tests passed concurrent reads, exact fake-file download, linked/redacted audit, exact-generation deletion, legal/HR holds, internal authorized notifications, and cleanup. The corrected false-positive workflow additionally passed exact clean rescan, requester self-approval denial, different-Admin approval, generation-bound exact download, four-event audit validation, and complete cleanup. A strict stored-event recomputation helper correction passes 91/91 locally and awaits its deployment decision. Remaining gates are existing-file planning, cost reconciliation, and an exact Production proposal. No Production or real-data action occurred.
