...
- Package 6 of 15, sensitive vault and safe release: **In progress / corrected first-scan image failed startup and was rolled back** on 2026-08-29. Existing sensitive release/audit, approval, retention, notification, false-positive, legacy, and Office gates remain credited. Immutable image `sha256:dca8aaa...` passed Artifact Analysis but its Staging revision failed before traffic because the Bash startup script was packaged with Windows CRLF bytes. The prior scanner digest is restored, temporary IAM is removed, and retry is paused. A local Dockerfile correction now normalizes and validates the startup script; it requires a newly approved image/scan, repeat narrow Staging installation, and the full certification/payroll regression matrix before billing reconciliation or a Production proposal. No Production or real-data action occurred.
