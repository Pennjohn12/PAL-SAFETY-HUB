...
- Package 6 of 15, sensitive vault and safe release: **In progress / partial Staging** on 2026-08-29. New W-4 and payroll/identity metadata separation, client-denied vault records, separate entitlement/purpose, two-person identity-document approval, and clean-only short-lived download policy are active in Staging. The approved keyless scanner identity and three isolated buckets now exist; scanner deployment is safely paused because the existing Firebase bucket cannot receive the required prefix-only permission without a broader storage-mode change. Approval for a fourth isolated quarantine bucket is pending. No real scanner or Production deployment exists.
