...
- Package 6 of 15, sensitive vault and safe release: **Production activation in progress / pre-test limit blocker** on 2026-08-30. Definitions, private scanner, twelve actions, rules, Hosting, and one private Eventarc path are deployed with retry/retention PAUSED and code-disabled. Before synthetic testing, read-only inventory verified the correct accepted digest, dedicated identity, 1 CPU, 4 GiB, concurrency 1, buckets, and callbacks, but found live scanner max scale 10 instead of the approved max 1. PAL stopped before creating/uploading any synthetic object or record. No file was released and Production is not complete; the limit must be corrected and reverified before the synthetic matrix.
