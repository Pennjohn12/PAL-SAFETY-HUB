...
- Package 6 trusted-rescan scanner digest `sha256:ab00939cb01de07150d20ad643dab338feb19fb31f0033049ad4a6f18e457da5` passed Artifact Analysis and is active only in private, min-0/max-1/concurrency-1 Staging. Three false-positive actions, private callback, and narrow identities/IAM are ACTIVE; anonymous boundaries fail closed. The corrected metadata handoff passed the complete authenticated two-person synthetic regression: clean exact-object rescan, same-person denial, different-Admin approval, generation-bound exact download, four-event audit validation, and complete cleanup. A strict stored-event audit recomputation helper passes 91/91 and requires no separate runtime deployment. The no-migration legacy-file treatment and future migration controls are documented without inspecting real data. Production and real data remain prohibited; billing reconciliation and an exact Production proposal remain open.
...
- Package 6 of 15, sensitive vault and safe release: **In progress / all isolated Staging functional gates and existing-file planning passed; billing and Production remain** on 2026-08-29. The accepted no-finding scanner runs privately against four isolated buckets and passed clean/quarantine, size, duplicate-generation, definition schedule, memory, timeout recovery, malformed-event, signed-download, two-person approval, audit, retention/legal-hold/deletion, notification, and false-positive release tests. The corrected false-positive workflow passed exact clean rescan, requester self-approval denial, different-Admin approval, generation-bound exact download, four-event audit validation, and complete cleanup. The no-migration legacy-file treatment is documented and preserves separate approval for any real-data inventory or migration. The strict stored-event verifier passes 91/91 and needs no separate runtime deployment. Remaining gates are billing reconciliation and an exact Production proposal. No Production or real-data action occurred.
