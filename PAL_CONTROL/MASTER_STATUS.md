...
- Package 6 startup-corrected scanner digest `sha256:5137ce2d9f744c428838fa67868c66fb180c342b3f9c21704150255b5b4b533b` passed its private bounded Staging startup/health gate on revision `pal-staging-malware-scanner-00011-mk8`. The subsequent synthetic certification scan returned CLEAN, but the isolated copied object had no destination security-envelope metadata, so the scanner rejected the callback as incomplete and the file stayed locked in `scan-queued`. Staging was rolled back to prior serving revision `pal-staging-malware-scanner-00010-p6g`; initial-callback Invoker and temporary unscanned viewer are removed and retry is PAUSED. A local Function-side exact-generation metadata write/read verification correction passes 99/99 PAL tests but is not deployed. Full lifecycle regressions remain required. Production and real data remain prohibited.
...
- Package 6 of 15, sensitive vault and safe release: **In progress / second fail-closed Staging integration blocker** on 2026-08-30. Existing sensitive release/audit, approval, retention, notification, false-positive, legacy, and Office gates remain credited. Immutable replacement `sha256:5137ce2d9f744c428838fa67868c66fb180c342b3f9c21704150255b5b4b533b` passed Artifact Analysis and its private bounded startup/health gate, but the first synthetic certification copy reached isolated storage without its required custom metadata. ClamAV returned CLEAN while the callback correctly failed closed as incomplete; nothing became downloadable. Rollback restored prior serving revision `pal-staging-malware-scanner-00010-p6g`, removed temporary callback/viewer access, and left retry PAUSED. A local Function-side envelope write/read verification correction passes 99/99 tests and needs focused review/deployment plus full synthetic regression. No Production or real-data action occurred.
