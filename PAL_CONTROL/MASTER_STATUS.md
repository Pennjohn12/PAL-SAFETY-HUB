...
- Package 6 startup-corrected scanner passed its private bounded Staging startup/health gate, but its first-scan reporter incorrectly required the false-positive-only `palOriginalIntakeId` instead of accepting the initial workflow's `palInitialScanIntakeId`. The exact copied object was reverified to contain the complete server-created envelope; the earlier contrary reading used incorrect CLI field names. The file stayed locked, Staging was rolled back to `pal-staging-malware-scanner-00010-p6g`, temporary IAM was removed, and retry is PAUSED. A local workflow-specific scanner correction passes fresh pinned build/audit and 41/41 scanner tests; a new image and scan remain separately approval-gated. Production and real data remain prohibited.
...
- Package 6 of 15, sensitive vault and safe release: **In progress / fail-closed first-scan reporter blocker corrected locally** on 2026-08-30. Existing release/audit, approval, retention, notification, false-positive, legacy, and Office gates remain credited. The copied object had its complete envelope, but the scanner demanded a false-positive-only intake field from the initial workflow. Nothing became downloadable; rollback restored `pal-staging-malware-scanner-00010-p6g`, removed temporary callback/viewer access, and left retry PAUSED. The workflow-specific local scanner correction passes fresh build/audit and 41/41 tests. A new immutable image/scan and full synthetic regression remain approval-gated; no Production or real-data action occurred.
