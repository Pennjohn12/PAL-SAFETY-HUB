...
- Package 6 trusted-rescan scanner digest `sha256:ab00939cb01de07150d20ad643dab338feb19fb31f0033049ad4a6f18e457da5` passed Artifact Analysis and is active only in private, min-0/max-1/concurrency-1 Staging. Three false-positive actions, private callback, and narrow identities/IAM are ACTIVE; anonymous boundaries fail closed. The latest authenticated regression proved Eventarc and scanning worked but exposed incorrectly nested copy metadata, so the scanner did not recognize the protected callback labels; all temporary data was cleaned up and the object stayed locked. The option-shape correction passes 91/91 locally but is not deployed. Production and real data remain prohibited; focused Staging deployment and a passing full regression are required before live false-positive completion, existing-file planning, billing reconciliation, or a Production proposal.
...
- Package 6 of 15, sensitive vault and safe release: **In progress / scanner, signed-download, two-person approval, live audit-chain, dedicated identity, retention, legal-hold, deletion, and internal-notification gates passed; false-positive handoff blocked** on 2026-08-29. The accepted no-finding scanner runs privately against four isolated buckets and passed clean/quarantine, size, duplicate-generation, definition schedule, memory, timeout recovery, and malformed-event tests. Three vault callables and the hourly retention worker use a dedicated zero-key least-privilege identity. Synthetic tests passed concurrent reads, different-person approval, exact fake-file download, linked/redacted audit, exact-generation deletion after the 24-hour/30-day windows, legal-hold and recent-review retention, internal authorized notifications with external delivery disabled, and complete cleanup. The latest false-positive run proved delivery and scanning but failed closed because the request action packaged PAL callback labels in the wrong Cloud Storage copy-option shape. The corrected shape and regression pass 91/91 locally but are not deployed. Remaining gates are focused Staging deployment and a passing live handoff, existing-file planning, cost reconciliation, and an exact Production proposal. No Production or real-data action occurred.
