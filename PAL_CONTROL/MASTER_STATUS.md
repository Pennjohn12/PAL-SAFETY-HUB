...
- Package 6 security scan passed for digest `sha256:945d6e425a7412fc3c0e89307d0e46f601554e4dce931be7be64ea575f075cef`, but pre-deployment review found its startup script still invoked npm after npm was removed from the runtime. The digest remains undeployed. A locally verified direct-Node startup repair requires a new `$0.26` scan before controlled synthetic Staging measurement; Production and real data remain prohibited.
...
- Package 6 of 15, sensitive vault and safe release: **In progress / partial Staging; scanner deployment blocked** on 2026-08-29. New W-4 and payroll/identity metadata separation, client-denied vault records, separate entitlement/purpose, two-person identity-document approval, and clean-only short-lived download policy are active in Staging. Four isolated scanner buckets, two keyless least-privilege identities, and a private image repository exist. The approved `$0.26` scan found 17 HIGH, 9 MEDIUM, and 6 unclassified records in the replacement image; all high and medium findings have available fixes. The repeated pinned build also produced a different digest, proving it is not bit-for-bit reproducible. No scanner runtime, real-data use, or Production deployment exists.
