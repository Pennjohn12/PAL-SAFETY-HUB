...
- Package 6 trusted-rescan scanner digest `sha256:ab00939cb01de07150d20ad643dab338feb19fb31f0033049ad4a6f18e457da5` passed Artifact Analysis and is active only in private, min-0/max-1/concurrency-1 Staging. A local/predeployment correction now automatically binds and queues both upload folders, records trusted clean/infected results through separate isolated-bucket events, retries interruptions, and gives Office a simple protected certification Open action while retaining stronger identity rules. The scanner image is unchanged and all 97/97 tests pass. Staging deployment/IAM and complete synthetic regressions, posted billing reconciliation, and the exact Production proposal/approval remain open; Production and real data remain prohibited.
...
- Package 6 of 15, sensitive vault and safe release: **In progress / first-scan correction predeployment** on 2026-08-29. Existing scanner, sensitive release/audit, approval, retention, notification, false-positive, legacy, and Office gates passed in Staging. The newly found first-scan/certification-release blockers now have a local candidate with exact-generation automatic queueing, trusted clean/infected output handling, ten-minute retry, a purpose-bound certification release callable, and simple no-action-needed UI; 97/97 tests pass. Required next gates are narrow Staging Eventarc/IAM/deployment and complete certification/payroll clean/infected/retry/release regressions, posted billing reconciliation, and only then an exact Production proposal/approval. No Production or real-data action occurred.
