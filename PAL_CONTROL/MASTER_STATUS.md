...
- Package 6 of 15, sensitive vault and safe release: **Production activation in progress / guarded synthetic verification next** on 2026-08-30. Definitions, private scanner, twelve actions, rules, Hosting, and one private Eventarc path are deployed with retry/retention PAUSED and code-disabled. Scanner service max is 1; Google documents the effective limit as the lower of service max 1 and ready-revision max 10, so the approved effective max 1 is enforced without a new revision. Digest, identity, resources, buckets, callbacks, and traffic remain unchanged. No synthetic object/record has entered the path yet and no file was released. Remaining pre-test gates and the approved synthetic matrix must pass before Production is complete.
