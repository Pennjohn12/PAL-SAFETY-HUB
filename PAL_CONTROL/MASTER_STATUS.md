...
- Package 6 exact Production activation was approved from commit `86449b355962437f51c353c88c972c5ea3e9d941`. The dedicated scan-active `us-east1/malware-scanner` repository, four empty private US-EAST1 buckets, and three zero-key Production identities exist with the approved narrow foundation IAM. Both Production `gcf-artifacts` repositories remain explicitly scan-disabled. After the original Docker path failed pre-manifest, John separately approved checksum-pinned official `gcrane` v0.22.0. It copied only accepted digest `sha256:e0de7bbb...d7cb`; independent gcrane and Google Cloud digest checks matched exactly. Artifact Analysis reached `FINISHED_SUCCESS` with zero vulnerability and zero malicious-package occurrences. The accepted Production-owned image remains undeployed; there is no scanner runtime or Package 6 Function/Hosting/rules deployment, real-data access, or release.
...
- The exact Package 6 Production proposal passed tests and Control Room review and John approved commit `86449b355962437f51c353c88c972c5ea3e9d941`. The foundation and Production-owned clean scanner image are established. Runtime/IAM callback, Function/rules/Hosting activation, and synthetic Production regression remain separately approval-gated and undeployed.
...
- Package 6 of 15, sensitive vault and safe release: **Production activation in progress / fail closed** on 2026-08-30. John approved exact delta `e8f6763...`; Phase 1 definitions, private scanner, twelve actions, rules, and Hosting deployed with retry/retention PAUSED and code-disabled. John then approved and PAL applied exact-bucket `roles/storage.bucketViewer` to the dedicated scanner identity; the role contains only bucket metadata get/list and no object permission. The single approved Eventarc retry failed while Google attempted to connect bucket notification metadata to its managed Pub/Sub topic. Read-only follow-up confirms zero Production Eventarc triggers, zero bucket notifications, and no matching leftover topic. No upload event path is active, no synthetic/real data was used, and files remain locked. Further mutation requires a newly reviewed exact Google service-agent/notification boundary.
