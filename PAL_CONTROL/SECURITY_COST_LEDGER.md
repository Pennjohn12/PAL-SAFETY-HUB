...
| 6 | Staging | Artifact Registry vulnerability scans for scanner measurement images | Console-disclosed price $0.26 per newly pushed image | $0.26 per newly pushed image; every replacement requires a new charge/approval record | Six scans authorized and incurred: $1.56 total; billing report not yet posted | First blocked by 17 high findings; second blocked by a critical name collision; third scanned clean but failed startup review; startup-fixed fourth, trusted-rescan fifth, and authenticated initial-callback sixth images passed with no findings |
...
- The last observed dashboard amount remains **$0.52 for the whole Staging project for Aug. 1–29**. Because it is project-wide and not broken down by SKU, it cannot yet be attributed fully to Package 6 or combined with `$1.56` as an invoice total.
...
- The authoritative current security-program direct-charge ledger is therefore **$1.56 known/incurred plus small usage-dependent Staging charges not yet posted or SKU-reconciled**. Production cost remains `$0` because no Package 6 Production resource exists.
