...
- PAL stopped again because the effective service-level cap may be one while the revision template remains ambiguous against the exact acceptance criterion. No second setting command, test object, or PAL record was used. Retry/retention remain PAUSED and code-disabled and no file was released; the Cloud Run service-versus-revision maximum distinction requires review before the synthetic matrix resumes.
