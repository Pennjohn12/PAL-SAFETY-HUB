...
- Local-only remediation now re-reads and validates the exact bucket object generation and complete PAL envelope immediately before callback, independently of mutable event metadata. Missing/mismatched evidence fails before file movement. Focused tests plus the full scanner build/39 tests and production audit=0 pass. Unique next tag is `pal-authoritative-metadata-1`; no push, scan charge, cloud permission, or deployment occurred.
