...
- Control Room classified focused `gcloud run services update ... --max 1` as correction within the already approved boundary. The command succeeded without creating a new revision and the service-level annotation remains `run.googleapis.com/maxScale: '1'`, but the ready revision template still reports `autoscaling.knative.dev/maxScale: '10'`.
- Control Room resolved the setting against current Google Cloud Run documentation: when service- and revision-level maxima both exist, the effective limit is the lower value. Service max 1 and revision max 10 therefore enforce effective max 1 immediately without a new revision. No `--max-instances` change is needed; digest, environment, identity, resources, and traffic remain unchanged. Cloud Run can briefly exceed maximum limits in limited circumstances, so the `$5` monitoring/stop boundary remains mandatory.
...
- Control Room review corrections explicitly separate the eight Firebase-authenticated public callable invocation bindings from private callbacks/schedules/job/scanner; add a default-disabled Production retry mode that returns before data access; require a manual authenticated definition update and verified fresh CVD objects before scanner startup; and distinguish already installed foundation IAM from the few resource-dependent grants still required.
- Final wording clarifies that the eight named callable services are platform-invokable so Firebase Authentication and application authorization can run; this does not permit anonymous application/data access or public invocation of callbacks, schedules, the updater job, or scanner.
- The delta begins with callbacks absent, retry PAUSED and code-disabled, retention PAUSED and code-disabled, and no real-data query. It excludes legacy enumeration/migration, real data, credentials, another image/build/scan, broader IAM, and retention enforcement/deletion.
...
- Final readiness corrections remove a duplicate finalizer service-account property and statically require exactly one; use bucket-scoped `storage.objectUser` for required invalid-upload deletion while retention remains disabled; replace every digest/IAM placeholder with literal policy; record exact Production Scheduler principals/targets from the live Staging OIDC/OAuth pattern; and define the scanner stop as a private min-0 service with Eventarc/callback/service Invokers removed, never `max-instances=0`.
