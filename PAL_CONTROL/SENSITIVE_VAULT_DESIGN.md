...
This design does not modify the scanner image and therefore does not require a sixth `$0.26` image analysis. The local suite passes 97/97 plus Functions/policy syntax. Live credit remains blocked until the exact Staging Functions, Eventarc triggers, schedule, narrow bucket permissions, and synthetic clean/quarantine/retry/download lifecycle pass with complete cleanup.

### Initial-scan concurrency and classification correction

Control Room review found two predeployment races in the local candidate. The corrected candidate now accepts a retry in `scan-result-recording` only when its result and isolated output generation exactly match the evidence already recorded. The chained result audit and a durable audit-completed marker are committed in one transaction, so a retry cannot create a second correlated result audit after a crash. Finalization requires that marker.

The timeout worker now transactionally re-reads each authorization and requires the same `scan-queued` state, queue timestamp, scan path, and scan generation observed by its query before marking it failed. A result that advances concurrently can no longer be overwritten by the scheduler.

The accepted scanner sends clean files to the clean bucket, while its quarantine bucket can include detected malware as well as encrypted or otherwise non-clean inputs. Because the unchanged image does not attach a trustworthy finer classification to ordinary first-scan output, the quarantine trigger records `manual-review`, never `infected`. The original remains locked. Scanner errors and timeouts produce no trusted output event and remain pending for the bounded retry/fail-closed path. A later exact scanner classification enhancement would require a changed image and a separately approved image scan.

Local regression coverage now proves matching result retries are idempotent, conflicting results or output generations fail closed, audit completion is once-only, stale queue transitions are transactionally guarded, and quarantine is labeled manual review. The full local suite passes 99/99 plus Functions syntax and diff checks. No cloud, IAM, credential, Production, real-data, or image-scan action occurred.

Independent review then found the queue worker had not persisted the isolated input object's generation, making the scheduler's generation comparison ineffective. The focused correction reads the destination metadata after either a successful exact-generation copy or a validated idempotent collision, requires a numeric generation, and stores it with the scan path and queue timestamp on both the record and authorization. Every requeue refreshes that generation. The stale transaction now rejects an absent/non-numeric or mismatched generation before changing state. Regression coverage verifies destination-generation capture and the guarded comparison; the full local suite passes 99/99.

### Staging output-writer IAM blocker

John approved the exact synthetic Staging installation and testing boundary. The mandatory read-only IAM inventory was completed before mutation on both `pal-safety-hub-staging-clamav-clean` and `pal-safety-hub-staging-clamav-quarantine`. Both have uniform bucket-level access and public-access prevention, but intended exclusive result writing is not satisfied.

The dedicated malware scanner has direct `Storage Object Admin`, as expected. However, both buckets also expose write-capable access through the project owners/editors legacy bucket/object owner grants and through the Firebase Admin SDK service account's inherited project-level `Storage Admin`. The intended vault result identity has not yet been granted bucket viewer access. Eventarc and other Google service agents are present through their normal project roles but were not credited as object writers merely from those unrelated roles.

Because an unintended write-capable identity could forge a clean-bucket object and trigger an authoritative result, no result trigger, first-scan Function, schedule, client, IAM grant, or Hosting component was deployed. Correcting inherited project IAM or creating a stronger isolated output boundary is a material permission/architecture decision requiring a separately reviewed plan and John's action-time approval. Production and real data remain unchanged.

### Private first-scan callback candidate

The local remediation removes clean/quarantine bucket finalization from the authorization trust boundary. The finalizer still creates the exact-generation scanner-input copy with the complete server envelope. The scanner hashes the bytes it actually scans and calls a new private first-scan endpoint using its existing keyless identity before moving the input to its operational clean/quarantine destination. Callback failure stops processing and leaves the PAL record locked and retryable.

The private endpoint accepts only the scanner's `clean` or `infected` protocol values, conservatively maps non-clean to `manual-review`, and verifies authorization ID, intake, folder, scanner-input path/generation, original Firebase path/generation/size/type/SHA-256, current pending record, result idempotency, and once-only chained audit before terminal state. Output buckets are evidence/operations only and cannot authorize release, so broader inherited output-bucket writers no longer forge PAL scan truth. The vault result identity needs no output-bucket permission and no new Eventarc result trigger is used.

The candidate preserves the already proven private false-positive callback and adds a separate `PAL_INITIAL_SCAN_CALLBACK_URL`. The scanner invokes the callback before moving the processed input. A fresh pinned-upstream preparation passed build, zero known production dependency audit findings, and 37/37 scanner tests. PAL Functions syntax, diff check, and 99/99 tests pass. No image was built or pushed, no Artifact Analysis charge was incurred, and no cloud/IAM/Production/real-data action occurred. The changed scanner image must stop for John's separate `$0.26` scan approval after exact patch provenance is checkpointed.

Independent review required three final binding corrections. The callback now requires the exact numeric scanner-input generation to equal the generation persisted by the queue for every first result. A terminal duplicate receives success only when its stored result, scanner-input path/generation, and both stored original identities exactly match; any conflicting terminal callback fails closed. Bounded ClamAV version and strict ISO scan time are required, retained in the immutable record evidence, and included in the chained audit reason while PAL's server completion time remains authoritative. Positive, mismatched-generation, and terminal-conflict regressions pass.

The final local rerun passed PAL 99/99 plus Functions syntax/diff, and a second fresh pinned-upstream preparation passed build, production dependency audit=0, and scanner 37/37. The scanner patch remains SHA-256 `8399f6cc260190f4e9d09bda3f2906181089d57304ea7fb4226568a842874e01`. No cloud, IAM, image push, scan charge, Production, or real-data action occurred.

John separately approved the corrected exact one-image boundary after the unique tag correction. Commit `89fbccef17064a974b8b239b9ce6a695d0e90d7a`, upstream `0db019c9f09494215aa4485b71094e9b8d5ea90b`, patch SHA-256 `6eea59d6f2c9d4c6210ebd7524bf42d4f0c5d673ff8455f731c998c7700c4673`, and tag `pal-initial-callback-1` were used. Dedicated keyless-builder Cloud Build `fb24262a-fa0a-41dd-9332-97311b56c6fe` succeeded and produced immutable digest `sha256:0de3ddbb8fb983e066f8a4b3bec16dd4ed5b34df78e74c7ee9b24ed73b5ca4e8`.

Artifact Analysis reached `FINISHED_SUCCESS`; all-metadata output contained package/discovery/image summaries and no vulnerability or malicious-package finding summary. The image meets the existing gate: no CRITICAL/HIGH and no fix-available MEDIUM findings or malicious-package match. The sixth `$0.26` scan is incurred, bringing authorized direct scans to `$1.56` pending invoice reconciliation. The image is accepted for the next separately approved synthetic Staging callback/runtime test only; it is not deployed and authorizes no IAM, Function, Hosting, Production, or real-data action.

A final read-only, digest-specific re-query before deployment again returned `FINISHED_SUCCESS`. The returned top-level metadata contained only discovery, image, and package summaries: the vulnerability-finding section was absent with an explicit occurrence count of `0`, and the malicious-package section was absent with an explicit occurrence count of `0`. No later finding appeared, no new image or scan charge was created, and the digest remains undeployed pending the exact Staging approval boundary.

### Authenticated first-scan Staging integration result

The approved Staging-only installation used accepted digest `sha256:0de3ddbb8fb983e066f8a4b3bec16dd4ed5b34df78e74c7ee9b24ed73b5ca4e8` on private bounded revision `pal-staging-malware-scanner-00007-9ll`, with the dedicated keyless scanner identity, both private callback URLs, and scanner-only Invoker on the new callback. The four focused Functions and Staging client deployed; no output-bucket trigger or clean/quarantine viewer grant was created. Both private endpoints returned anonymous HTTP `403`.

The first synthetic certification exposed two safe integration findings. First, the Storage client mutates the supplied custom-metadata object, while PAL intentionally freezes its policy envelope. The server now gives Storage a shallow copy, preserving the frozen trusted source and every field; a regression asserts this packaging and all 99 PAL tests pass. Second, exact destination-generation lookup requires the vault runtime to view metadata on only the isolated unscanned bucket. A temporary bucket-specific viewer grant proved generation persistence and was removed during rollback.

After those corrections, the exact 41-byte PDF and all required initial-scan metadata reached the isolated unscanned bucket. ClamAV repeatedly returned CLEAN, but the scanner rejected callback preparation as `PAL scan callback metadata is incomplete`. Independent object inspection showed authorization, intake, folder, scan path, original path/generation/size/content type/SHA-256, and the private callback configuration present. No private callback was accepted, no trusted result/audit was recorded, and release remained impossible. This is a fail-closed scanner runtime metadata-consumption blocker, not a confidentiality failure.

Rollback restored prior accepted digest `sha256:ab00939cb01de07150d20ad643dab338feb19fb31f0033049ad4a6f18e457da5` on revision `pal-staging-malware-scanner-00008-rkc`, removed the new callback environment and Invoker binding, paused the initial-scan retry schedule, removed the temporary unscanned viewer grant, and cleaned the named synthetic records/objects. Production and real data were untouched. A corrected scanner must obtain and validate authoritative exact-generation object metadata rather than relying on the incomplete runtime event representation; any changed image requires fresh immutable provenance and John's separate `$0.26` Artifact Analysis approval before push.

The local correction now performs a fresh server read of the exact named object generation immediately before either PAL callback. It rejects an empty/non-numeric or changed generation, missing custom envelope, non-string values, overlong fields, and excessive fields; it freezes a copy of the authoritative custom fields and ignores mutable caller/event metadata. Callback-required treatment is selected from the PAL-controlled `initial-scans/<id>/<file>` and `false-positive-rescans/<id>/<file>` path boundary, never from preliminary metadata, so missing event metadata cannot take the ordinary-file route. Genuinely ordinary paths remain unchanged. Callback reporting still occurs before clean/quarantine movement, so lookup or callback failure leaves the object unmoved and unreleased. Focused tests cover path classification, exact bucket/name/generation selection, caller-metadata independence, frozen evidence, malformed envelope denial, generation mismatch, and missing-envelope denial; the full scanner build and 40/40 tests, PAL 99/99, production dependency audit at zero, and commit-range whitespace check pass. The exact checked-in patch SHA-256 is `f635b23a456284b59c2296a9aa657064ed97bdef2745ef4f09536ca9c9494fc8`; the unique candidate tag is `pal-authoritative-metadata-1`. This is local-only: no image, push, scan charge, IAM, runtime, Staging, Production, or real-data action is authorized.

John then approved exactly one image build/push and one `$0.26` Artifact Analysis scan from that reviewed source. Dedicated-builder Cloud Build `ebd3423f-9050-4da0-9ce1-507d9c4d2d74` succeeded and produced immutable digest `sha256:dca8aaa4128be626b655766374d4bf10a999d95cbf78e20669fbc1d28031d19d` under tag `pal-authoritative-metadata-1`. The digest-specific package-vulnerability response contained only discovery/image summaries and zero vulnerability findings. The all-metadata response contained discovery/image/package summaries and no vulnerability or malicious-package occurrence section. The image therefore passes the existing no-CRITICAL/HIGH/no-fixable-MEDIUM/no-malicious-package gate. The seventh scan is incurred and cumulative authorized direct scan charges are `$1.82` pending reconciliation. The digest is accepted but **undeployed**; a separately reviewed and approved Staging runtime/IAM/regression boundary remains required.

The approved Staging installation then exposed a packaging-only startup defect before the new revision received traffic. Revision `pal-staging-malware-scanner-00009-mvq` logged `$'\r': command not found` and a Bash syntax error from CRLF bytes in `bootstrap.sh`, then failed its Cloud Run startup probe. No callback, trusted result, or release occurred. Rollback restored prior accepted digest `sha256:ab00939c...` on serving revision `pal-staging-malware-scanner-00010-p6g`, removed the scanner-only initial-callback Invoker and isolated-unscanned objectViewer, removed the initial callback environment value, and left the retry Scheduler PAUSED. The focused Functions and Hosting remain deployed but fail closed without those runtime permissions.

The local candidate now makes the Linux image build normalize CRLF only from `bootstrap.sh`, explicitly rejects any remaining carriage-return byte, and runs `bash -n bootstrap.sh` before dependency installation. The preparation harness requires those Dockerfile controls in addition to its local startup syntax check and forces the pinned clone to retain LF source bytes. This correction does not change callback authorization or scanning logic. Exact patch SHA-256 is `cb01e05bda922ea760bd6244c4ad616b29e078217d4cb3c8e64168457d699207`; its unique tag is `pal-authoritative-metadata-startup-1`. A fresh pinned-source preparation passed production audit=0, TypeScript build, 40/40 scanner tests, PAL 99/99, and diff check. It must stop for a separately approved additional `$0.26` image scan. Production remains closed.

John approved that exact one-image boundary. Dedicated-builder Cloud Build `de2738dd-02db-4c9b-8880-20604bb2d897` succeeded and produced immutable digest `sha256:5137ce2d9f744c428838fa67868c66fb180c342b3f9c21704150255b5b4b533b` under tag `pal-authoritative-metadata-startup-1`. Digest-specific package-vulnerability output contained only discovery/image summaries; all-metadata output contained discovery/image/package summaries and no vulnerability or malicious-package occurrence section. The image passes the existing acceptance gate. The eighth scan is incurred, for `$2.08` cumulative direct scan charges pending billing reconciliation. The image remains **undeployed**; startup/health must be the first gate in a separately approved repeat Staging installation before callback IAM or retry is restored.

### 2026-08-30 startup pass and destination-envelope fail-closed checkpoint

The approved Phase 1 installed only digest `sha256:5137ce2d9f744c428838fa67868c66fb180c342b3f9c21704150255b5b4b533b` on private revision `pal-staging-malware-scanner-00011-mk8`. It passed health, serving, exact-digest/configuration, anonymous 403, and successful Bash/ClamAV/application startup checks while callback Invoker was absent and retry stayed PAUSED. Phase 2 then configured the private callback on revision `pal-staging-malware-scanner-00012-28s` and applied only scanner-to-callback Invoker plus vault-runtime objectViewer on the isolated unscanned bucket.

The first synthetic certification proved public grant/upload/finalization, isolated copy, Eventarc, and a CLEAN ClamAV result, but callback preparation failed closed as incomplete. The authorization stayed `scan-queued`, the certification stayed pending and non-downloadable, and no clean trust was recorded. Rollback restored 100% traffic to `pal-staging-malware-scanner-00010-p6g`, removed initial callback Invoker and temporary unscanned viewer, and left retry PAUSED. Synthetic-only cleanup completed; audit evidence was not rewritten.

The initial metadata conclusion was then corrected. The CLI exposes custom labels under `custom_fields` and uses `content_type` and `cache_control`; exact-generation CLI and Node Storage reads proved all required custom fields, `application/pdf`, and `private, no-store, max-age=0` were present. A temporary exact-generation `setMetadata()` addition failed with HTTP 403 under the intended least-privilege runtime and was reverted; the original focused finalizer was redeployed. The true cause was in `pal-rescan-reporter.ts`: one shared required list included `palOriginalIntakeId`, which exists for false-positive rescans but intentionally does not exist for initial scans that use `palInitialScanIntakeId`.

The local scanner correction now requires `palOriginalIntakeId` only for false-positive callbacks and requires `palInitialScanIntakeId` only for initial-scan callbacks. Tests prove a complete initial envelope succeeds without the false-positive field, a missing initial intake fails, and a false-positive envelope still cannot omit its original intake. Exact-generation authoritative lookup, bounded metadata validation, digest/path binding, callback-before-move ordering, and ordinary-file behavior remain unchanged. A fresh clone of pinned upstream passed production audit=0, TypeScript build, and 41/41 scanner tests; Functions syntax, commit diff, and 99/99 PAL tests also pass. The exact checked-in patch SHA-256 is `a14d7836af49e4efc211f93da466164c425b421d311d8e17274a3129e458df07`; the candidate tag is `pal-initial-intake-field-1`. No image, push, scan charge, runtime, IAM, Staging, Production, or real-data action is authorized by this local correction.

John approved exactly one image build/push and one `$0.26` Artifact Analysis scan from that checkpoint. Dedicated keyless-builder Cloud Build `f8dfaf77-f26d-4f75-a08e-ad01bc4596da` succeeded and produced immutable digest `sha256:e0de7bbb029eb9d342bd56b9e215fab33d39f27eb3fe23f42728ed920cedd7cb` under tag `pal-initial-intake-field-1`. Digest-specific package-vulnerability output contained only discovery/image summaries, no finding paths, and zero occurrences; all-metadata output contained discovery/image/package summaries, no vulnerability or malicious-package finding paths, and zero occurrences. The ninth scan is incurred, bringing direct scan charges to `$2.34` pending reconciliation. The image is accepted but **undeployed**; repeat Staging runtime/IAM/regression is a later separately approved boundary. Production remains closed.

### Repeat lifecycle regression and scanner-detection blocker

John approved the isolated repeat Staging installation and regression. Accepted digest `sha256:e0de7bbb...` became healthy on the private min-0/max-1/concurrency-1 scanner with only the approved scanner callback Invoker and exact isolated-unscanned viewer access. A test-command error first supplied the Production Firebase web identifier; Firebase rejected those tokens for Staging and the harness cleaned them. The corrected command proved Staging-issued token audience/issuer before protected calls.

The corrected live matrix passed exact-object certification first scan, purpose-bound exact-byte certification download, payroll/identity clean scan, non-entitled denial, same-person approval denial, different-Admin approval, and exact-byte generation-bound identity download. The direct certification record also showed numeric scanner-input generation binding and a completed once-only audit transition.

The non-clean gate did not pass. The first harmless antivirus fixture was malformed and was discarded as evidence. After replacing it with the exact standard synthetic test signature, the scanner still returned `clean`; the server therefore recorded `verified-clean` instead of `scan-manual-review`. The harness stopped on its bounded wait and cleaned all nine documents, three objects, and three temporary users. This is a verified scanner detection/security blocker even though authorization and cleanup remained fail closed operationally and no real data was used.

Rollback verification restored 100% traffic to `pal-staging-malware-scanner-00010-p6g`, removed scanner Invoker from the initial callback, removed the temporary vault-runtime objectViewer grant while preserving its pre-existing objectCreator role, kept retry `PAUSED`, and removed the earlier direct synthetic authorization/intake and exact objects. Production remains unchanged. Before another runtime attempt, the scanner engine/definition loading and freshness evidence must explain the missed standard test signature, then demonstrate both a clean control and a manual-review control. Any changed image still requires its own immutable build and separately approved paid Artifact Analysis boundary.

### Detection-evidence correction and verified encrypted-file control

The preceding detection-blocker conclusion was later disproved as test evidence. The purported corrected EICAR object was 69 bytes because it retained an extra `)`, and the string followed a PDF header instead of starting at byte zero. It therefore did not satisfy the official exact-68-byte/start-of-file requirement. Its CLEAN result cannot establish a scanner weakness, and PAL explicitly retracts that finding while preserving the original record as an audit trail.

A new 912-byte, one-page synthetic PDF was encrypted with a synthetic password. Local `pypdf` verification proved encryption, denial without the password, successful password decryption, and one readable page. SHA-256 `ecefc9834eb1772c43ce5f5302386a860b90c353e531bb4d7c3e4a0d72bd4a8f` matched exactly after upload to the isolated test environment. The accepted image ran temporarily on private revision `pal-staging-malware-scanner-00015-qvl` with only the previously approved scanner Invoker and isolated-unscanned viewer; retry remained PAUSED.

Final run `PAL-SYNTHETIC-P6-40f06bd9fc57` passed the separated lifecycle. Certification and payroll/identity clean files reached their proper release paths, with purpose, entitlement, different-person identity approval, generation binding, and exact 73-byte download checks. ClamAV 1.5.2 with definition version 28107 logged encrypted-document alerting enabled and `Heuristics.Encrypted.PDF FOUND` for the exact 912-byte object in 84 ms. PAL recorded only `scan-manual-review`; the file remained non-downloadable. Anonymous callback access returned `403`, six audit events passed redaction checks, Staging Hosting returned `200`, and a deployed-source probe returned `404`.

Cleanup removed nine temporary documents, three objects, and three synthetic users. Rollback restored 100% traffic to `pal-staging-malware-scanner-00010-p6g`, removed callback Invoker and temporary objectViewer while preserving objectCreator, verified retry `PAUSED`, and removed temporary Cloud Shell fixture/log/harness copies. No Production, real-data, credential, image, or paid-scan action occurred. Scanner detection/manual-review is now credited in isolated Staging; retry-recovery, posted billing reconciliation, and the exact Production proposal remain open.

### Initial-scan retry recovery and terminal idempotency

A temporary synthetic harness with matching local/Staging SHA-256 `41010b5feee6dea370fdb60c32048b9cf0e8175e04c9af31ea2dacb1d40dcde0` created one exact certification object, intake record, and authorization already in locked `scan-queue-failed` state. Before recovery the record was `pending` and `downloadable: false`. The first manual force attempt correctly failed because Google Scheduler requires an ENABLED job; it created no trust, and cleanup removed that attempt's synthetic data.

The retry job was then briefly resumed only in Staging, the accepted private scanner and the same two narrow temporary permissions were restored, and synthetic run `PAL-SYNTHETIC-RETRY-0ce7fb3a93fc` forced recovery. The worker copied and persisted the exact scanner-input generation `1788070477868234`; the accepted scanner returned clean; authorization advanced to `scan-clean`; the record retained `downloadable: false` and the original SHA-256 in trusted evidence. Exactly one correlated scan-result audit event existed.

A second forced retry left the terminal state, audit-event ID, and correlated audit count unchanged at one. The job was immediately paused. Cleanup removed the named authorization, intake, original Firebase object, and matching isolated unscanned/clean/quarantine object. Rollback restored 100% traffic to `pal-staging-malware-scanner-00010-p6g`, removed callback Invoker and temporary objectViewer while preserving objectCreator, and removed temporary harness/log copies. No Production, real-data, credential, image, or paid-scan action occurred. Retry recovery and terminal idempotency are credited; posted billing reconciliation and the exact Production proposal remain open.

### Posted billing reconciliation and cost-control hold

The Aug. 1–29 Staging billing report now posts 40 units / `$10.40` under Container Images Scanned (SKU `BEA5-E1D1-4659`). This supersedes the earlier whole-project dashboard estimate. Nine dedicated scanner-candidate scans / `$2.34` were individually approved, leaving a 31-unit / `$8.06` variance from the approved-action expectation. Current read-only Artifact Registry inventory contains 10 unique digests in the dedicated scanner repository and 36 in the Cloud Functions-managed repository; billing has not yet posted enough repository-level detail to map each of the 40 units to an exact digest.

The verified cause class is project-wide automatic image scanning combined with repeated scanner and focused Cloud Functions deployments. It does not indicate a data leak or Production security failure, but it exceeded the earlier `$5` scanner-specific stop expectation and shows that the cost boundary was defined too narrowly. All discretionary Staging build, deploy, scan, and runtime work is stopped. No image was deleted and no scanning policy was changed. Package 6 cannot proceed to a financially accurate Production proposal until John approves an exact design that preserves mandatory scanning of PAL's dedicated malware-scanner candidates while preventing unrelated deployment images from silently entering the paid scan scope.

John approved the repository-scoped design. Both Cloud Functions-managed `gcf-artifacts` repositories (`us-central1` and `us-east1`) now verify automatic scanning disabled, while the dedicated `us-east1/malware-scanner` repository verifies scanning active. This preserves PAL's immutable scanner-image acceptance gate and removes unrelated focused Function deployments from the intended paid scan scope. The explicit tradeoff is loss of Google's automatic container-vulnerability visibility for Staging Function images; PAL's source/dependency audits and regression checks remain mandatory but do not replace that lost container-layer signal. Re-enabling scanning on both named Function repositories is the documented rollback and requires a renewed cost/security decision. No Production or data-path control changed.
