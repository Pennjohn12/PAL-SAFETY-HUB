...
The first synthetic certification proved public grant/upload/finalization, isolated copy, Eventarc, and a CLEAN ClamAV result. It also proved a new blocker: the exact isolated object generation had no custom metadata and no requested content type/cache control. The scanner's authoritative lookup therefore rejected the result as incomplete before callback acceptance or movement. The authorization stayed `scan-queued`, the certification stayed pending and non-downloadable, and no clean trust was recorded. Rollback restored 100% traffic to `pal-staging-malware-scanner-00010-p6g`, removed initial callback Invoker and temporary unscanned viewer, and left retry PAUSED. Synthetic-only cleanup completed; audit evidence was not rewritten.

The local correction treats copy completion alone as insufficient. It applies the complete envelope to the exact copied generation, re-reads that generation, and verifies generation, content type, private/no-store cache control, and every custom security field before writing `scan-queued`. A scanner event that races the metadata write can only fail and retry because the callback still requires a complete authoritative envelope and a queued exact generation. Syntax, diff, and 99/99 PAL tests pass. This correction remains predeployment; full certification clean, payroll clean, non-clean/manual-review, retry/idempotency, purpose-bound certification release, two-person identity release, audit/redaction, UI, and cleanup evidence remain required before any Production proposal.
