...
The local correction now performs a fresh server read of the exact named object generation immediately before either PAL callback. It rejects an empty/non-numeric or changed generation and a missing custom envelope, freezes a copy of the authoritative custom fields, and ignores mutable caller/event metadata. Only PAL-tagged objects perform this additional lookup; ordinary scanner behavior is unchanged. Callback reporting still occurs before clean/quarantine movement, so lookup or callback failure leaves the object unmoved and unreleased. Focused tests cover exact bucket/name/generation selection, caller-metadata independence, frozen returned evidence, generation mismatch, and missing-envelope denial; the full scanner build and 39/39 tests pass with production dependency audit at zero. Candidate tag is uniquely changed to `pal-authoritative-metadata-1`. This is local-only: no image, push, scan charge, IAM, runtime, Staging, Production, or real-data action is authorized.
