...
- In progress: **Package 6**. Sensitive-record separation and authorization are active in Staging. The accepted no-finding scanner runs privately against four isolated synthetic buckets; clean/quarantine, size, duplicate-generation, definition refresh, forced schedule, peak memory, timeout fail-closed/recovery, and malformed-event paths passed. Dedicated-identity signed download, different-person approval, tamper-evident audit, exact-generation retention deletion, legal/HR holds, internal entitled notifications, and the complete false-positive release workflow have now passed with synthetic cleanup. The false-positive pass includes exact clean rescan, requester self-approval denial, different-Admin approval, generation-bound exact download, and four-event audit validation. A strict stored-event recomputation helper correction passes 91/91 locally. Existing-file planning, billing reconciliation, that helper's deployment decision, and Production remain open in `SENSITIVE_VAULT_DESIGN.md`.
