...
- In progress: **Package 6**. Existing scanner, sensitive release/audit, approval, retention, notification, false-positive, legacy, and Office controls passed in isolated Staging. The first-scan/certification-release blocker now has a local automatic candidate: exact-generation queueing for both folders, trusted clean/infected result events, retry without employee action, and simple purpose-bound certification release while identity keeps stronger rules. All 97/97 tests pass. Narrow Staging Eventarc/IAM/deployment and full lifecycle regressions remain required before billing reconciliation and an exact Production proposal; Production remains closed.
