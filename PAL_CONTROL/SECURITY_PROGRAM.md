...
- In progress: **Package 6**. Sensitive-record separation and authorization are active in Staging. The accepted no-finding scanner runs privately against four isolated synthetic buckets; clean/quarantine, size, duplicate-generation, definition refresh, forced schedule, peak memory, timeout fail-closed/recovery, and malformed-event paths passed. Dedicated-identity signed download, different-person approval, tamper-evident audit, exact-generation retention deletion, legal/HR holds, and internal entitled notifications also passed with complete synthetic cleanup. The latest false-positive regression proved object delivery and clean scanning but failed closed because the request action nested the PAL callback labels under the wrong Cloud Storage copy option. A corrected option shape passes 91/91 locally and awaits focused Staging deployment and the full live regression. Existing-file planning, billing reconciliation, and Production also remain open in `SENSITIVE_VAULT_DESIGN.md`.
