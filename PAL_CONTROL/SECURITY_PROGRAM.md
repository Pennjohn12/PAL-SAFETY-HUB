...
- In progress: **Package 6**. Existing scanner, sensitive release/audit, approval, retention, notification, false-positive, legacy, and Office controls passed in isolated Staging. The authoritative-metadata replacement passed Artifact Analysis but failed its Staging startup probe before traffic because its Bash script contained Windows CRLF bytes; the prior scanner and fail-closed IAM/schedule state were restored. A local image-build correction now normalizes and validates that startup script. A separately approved replacement image/scan, repeat narrow Staging installation, and full lifecycle regressions remain required before billing reconciliation and an exact Production proposal; Production remains closed.
