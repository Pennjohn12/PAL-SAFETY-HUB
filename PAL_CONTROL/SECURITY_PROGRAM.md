...
- In progress: **Package 6**. Existing scanner, sensitive release/audit, approval, retention, notification, false-positive, legacy, and Office controls passed in isolated Staging. The first-scan/certification candidate now has an accepted, undeployed immutable scanner image using exact-generation queueing and a private authenticated callback rather than output-bucket authorization. Retry remains invisible to employees, certification release stays simple, and identity keeps stronger rules. Narrow Staging callback/runtime deployment and full lifecycle regressions remain required before billing reconciliation and an exact Production proposal; Production remains closed.
