...
- In progress: **Package 6**. Existing sensitive release/audit, approval, retention, notification, false-positive, legacy, and Office controls passed in isolated Staging. The repeat first-scan lifecycle proved certification clean/release, payroll/identity clean/two-person release, locked manual review for a verified encrypted PDF, and automatic retry recovery with a once-only terminal audit. Staging is rolled back to `pal-staging-malware-scanner-00010-p6g`, temporary callback/viewer access is removed, retry is PAUSED, and named synthetic data is cleaned. Posted billing shows 40 image-scan units / `$10.40`, 31 units / `$8.06` beyond the nine explicitly approved scanner-candidate scans. John approved repository-scoped control: both Staging Cloud Functions repositories now have automatic scanning disabled, while the dedicated malware-scanner repository remains active. The exact Production proposal is now prepared for testing/independent review but remains unapproved and undeployed. Production remains closed.
