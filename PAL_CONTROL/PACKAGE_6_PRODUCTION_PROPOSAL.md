...
docker image inspect us-east1-docker.pkg.dev/pal-safety-hub-staging/malware-scanner/malware-scanner@sha256:e0de7bbb029eb9d342bd56b9e215fab33d39f27eb3fe23f42728ed920cedd7cb --format="{{json .RepoDigests}}"
docker tag us-east1-docker.pkg.dev/pal-safety-hub-staging/malware-scanner/malware-scanner@sha256:e0de7bbb029eb9d342bd56b9e215fab33d39f27eb3fe23f42728ed920cedd7cb us-east1-docker.pkg.dev/pal-safety-hub/malware-scanner/malware-scanner:pal-package6-prod-1
...
| `service-461653262208@serverless-robot-prod.iam.gserviceaccount.com` | Production `malware-scanner` repo | Repo absent | Explicit repository-scoped `roles/artifactregistry.reader` | Pull accepted digest for Cloud Run | Persistent while scanner exists; remove on scanner rollback |
...
| Same vault identity | Existing Production Firebase Storage bucket | No dedicated grant | Bucket-scoped `roles/storage.objectUser` | Verify/hash exact source generation, authorize exact clean download, and delete only invalid completed uploads; later retention deletion remains code-disabled | Persistent operational grant; includes object delete capability constrained by exact-generation code and disabled retention; remove on rollback |
...
| `service-461653262208@gcp-sa-cloudscheduler.iam.gserviceaccount.com` | Project | Google-managed agent may be absent until API use | Google-managed `roles/cloudscheduler.serviceAgent`; no custom broad role | Mint tokens for exact Scheduler targets | Preserve service-agent role; pause/delete only PAL jobs on rollback |
| `pal-prod-vault-download@pal-safety-hub.iam.gserviceaccount.com` | `retryPendingInitialScansV1` and `enforceSensitiveVaultRetentionV1` exact generated Cloud Run services | Services absent | `roles/run.invoker` on those two services only | OIDC principal for `firebase-schedule-retryPendingInitialScansV1-us-central1` and `firebase-schedule-enforceSensitiveVaultRetentionV1-us-central1` | Persistent while schedules exist; pause jobs/remove target grants on rollback |
| `pal-prod-clamav-updater@pal-safety-hub.iam.gserviceaccount.com` | Exact Cloud Run job `pal-prod-cvd-update` | Job absent | `roles/run.invoker` on that job only | OAuth principal for Scheduler job `pal-prod-cvd-update` in `us-east1` | Persistent while schedule exists; pause job/remove target grant on rollback |

Before mutation, export project/repository/bucket/service IAM, service accounts/key counts, Functions and revisions, Scheduler jobs/states, Eventarc triggers, Hosting versions, Firestore/Storage rules, and relevant API states. The Scheduler design is grounded in live Staging evidence: retry and retention use the dedicated vault identity through OIDC, while the CVD update uses a dedicated job-invocation identity through OAuth; Production uses the exact named equivalents above. After each grant, compare the policy to this table; any extra principal or broader role stops the release.
...
- Function-container automatic scanning: disabled on both Production `gcf-artifacts` repositories, matching approved Staging Option A. This saves unpredictable per-deployment scan charges but loses Google's automatic Function-container vulnerability signal. Compensating controls are production dependency audits, syntax checks, 101 PAL regressions, focused source review, exact lockfile deployment, and post-deploy behavior checks.
...
- Create a dedicated `$5` Production Package 6 budget alert/monitoring threshold before runtime activation. A budget alert is not a hard cap. If incremental Package 6 estimated/posting cost reaches `$5` before PAL reviews it, pause retry, definition-update, and retention schedules; disable the Eventarc trigger; remove both callback Invoker grants and the Eventarc service-to-scanner Invoker path; keep the private min-0 scanner deployed with no remaining invocation path so it naturally scales to zero; and leave every file locked. Never use `max-instances=0` as a stop control.
...
- Disable the new Eventarc trigger; remove both scanner callback Invoker grants and every service-to-scanner Invoker path; leave the private min-0 scanner deployed with no invocation path so it naturally scales to zero. Do not use `max-instances=0`. Leave queued files locked.
...
- Confirm that no invocation paths remain and that the private min-0 scanner has zero active instances. Retain the accepted image/repository for evidence; do not delete it during emergency rollback.
